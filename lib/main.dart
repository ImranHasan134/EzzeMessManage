      ...
      build: (ctx) => [
        ...
        ...summary.members.map((ms) => pw.Column(
          ...
          children: [
            ...
            _pdfRow(
              ms.hasDue ? 'Due' : ms.isOverpaid ? 'Advance' : 'Settled',
              '৳${ms.due.abs().toStringAsFixed(0)}',
              bold: true,
              color: ms.hasDue ? PdfColors.red700 : ms.isOverpaid ? PdfColors.blue700 : PdfColors.green700,
            ),
...
class ImportService {
  Future<ImportResult> pickFile() async {
    ...
    try {
      ...
      return ImportResult(success: true, message: 'Ready to import ${backup.monthId}', backup: backup);
    } catch (e) {
  ...
  void commitImport(MessBackup backup) {
    ...
    for (final m in backup.members)      { _memberBox.put(Member.fromJson(m)); }
    for (final m in backup.mealEntries)  { _mealBox.put(MealEntry.fromJson(m)); }
    for (final b in backup.bazarEntries) { _bazarBox.put(BazarEntry.fromJson(b)); }
    for (final c in backup.otherCosts)   { _costBox.put(OtherCost.fromJson(c)); }
    for (final p in backup.payments)     { _paymentBox.put(Payment.fromJson(p)); }
  }
}
    ...
    return ThemeData(
      ...
      cardTheme: CardThemeData(
        ...
        shape: RoundedRectangleBorder(
          ...
          side: BorderSide(
              color: dark ? Colors.white.withOpacity(0.08) : Colors.black.withOpacity(0.08)),
        ),
        ...
        child: Column(
          ...
          children: [
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    _drawerTile('History', Icons.folder_rounded, 5),
            ...
            Padding(
              ...
              child: Text('v1.0.0 · Local-first · ObjectBox',
...
class _HomeScreenState extends State<HomeScreen> {
          ...
          children: [
            if (_summary != null) ...[
              ...
              Row(children: [
                ...
                const Spacer(),
                Text('${_summary!.members.length} active',
                ...
                const Card(
                  child: Padding(
                    ...
                    child: Column(children: [
                      ...
                      Text('Go to Settings → Add member first.',
  ...
  Widget _buildSummaryCards(MonthSummary s) {
    ...
    return Column(children: [
      Row(children: [
        Expanded(child: _metricCard('Meal rate', '৳${s.mealRate.toStringAsFixed(2)}', 'per meal')),
        const SizedBox(width: 8),
        Expanded(child: _metricCard('Total meals', '${s.totalMeals}', 'this month')),
      ]),
      const SizedBox(height: 8),
      Row(children: [
        Expanded(child: _metricCard('Bazar spent', '৳${s.totalBazar.toStringAsFixed(0)}', widget.monthId)),
        const SizedBox(width: 8),
        Expanded(child: _metricCard('Bazar budget', '৳${s.bazarBudget.toStringAsFixed(0)}', 'paid − fixed costs')),
      ]),
      const SizedBox(height: 8),
      _metricCard(
        'Bazar remaining',
  ...
  Widget _buildMemberCard(MemberSummary ms) {
    final color = ms.hasDue ? Colors.red : ms.isOverpaid ? Colors.blue : Colors.green;
    final label = ms.hasDue
        ? '৳${ms.due.toStringAsFixed(0)} due'
        : ms.isOverpaid ? '+৳${ms.due.abs().toStringAsFixed(0)}' : 'Settled';
    ...
    return Card(
      ...
      child: ListTile(
        leading: CircleAvatar(
          ...
          child: Text(ms.member.initials, style: TextStyle(color: color, fontWeight: FontWeight.bold)),
        ),
        title: Text(ms.member.name, style: const TextStyle(fontWeight: FontWeight.w500)),
        ...
        trailing: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Text(
                ms.hasDue ? 'due' : ms.isOverpaid ? 'overpaid' : 'paid',
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          _detailRow('Meal cost (${ms.totalMeals} meals)', '৳${ms.mealCost.toStringAsFixed(0)}'),
          ...
          _detailRow(
            ...
            '৳${ms.due.abs().toStringAsFixed(0)}',
            bold: true,
            color: ms.hasDue ? Colors.red : ms.isOverpaid ? Colors.blue : Colors.green,
          ),
    ...
    return Padding(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(label, style: TextStyle(
            color: Colors.grey, fontWeight: bold ? FontWeight.bold : FontWeight.normal)),
        Text(value, style: TextStyle(
            color: color, fontWeight: bold ? FontWeight.bold : FontWeight.normal)),
      ]),
    );
  }
}
...
class _MealScreenState extends State<MealScreen> {
  ...
  void _load() {
    ...
    setState(() {
      ...
      for (final m in members) { updatedCounts[m.id] = _counts[m.id] ?? 0; }
      _counts..clear()..addAll(updatedCounts);
  ...
  void _save() {
    ...
    for (final member in _members) {
      ...
      if (count > 0) {
        _db.saveMeal(MealEntry(memberId: member.id, count: count, date: _date, monthId: widget.monthId));
        saved++;
      }
    }
    if (saved == 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Nothing to save — all counts are 0'), backgroundColor: Colors.orange));
      return;
    }
    for (final key in _counts.keys.toList()) { _counts[key] = 0; }
    _load();
  ...
  int _mealCountOn(int memberId, int day) {
    ...
    return _allMonthEntries
        .where((e) => e.memberId == memberId && e.date.year == year &&
        e.date.month == month && e.date.day == day)
        ...
        actions: [
          Padding(
            ...
            child: TextButton.icon(
              ...
              icon: Icon(_showCalendar ? Icons.list_rounded : Icons.grid_view_rounded, size: 18),
      ...
      return Center(
        child: Padding(
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const SizedBox(height: 16),
            const Text('No members yet', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
            const SizedBox(height: 8),
            const Text('Go to Settings → Add member first.',
          ...
          child: Column(children: [
            Padding(
              ...
              child: Row(children: [
                ...
                SizedBox(width: 120,
                    child: Text('Meals today', textAlign: TextAlign.center,
            ...
            ...List.generate(_members.length, (i) {
              ...
              return Container(
                decoration: BoxDecoration(border: Border(bottom: i < _members.length - 1
                    ? BorderSide(color: Colors.grey.withOpacity(0.15), width: 0.5) : BorderSide.none)),
                padding: const EdgeInsets.symmetric(vertical: 4),
                child: Row(children: [
                  CircleAvatar(
                    radius: 18,
                    backgroundColor: Theme.of(context).colorScheme.primaryContainer,
                    child: Text(m.initials, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold,
                  ...
                  Material(
                    ...
                    child: InkWell(
                      ...
                      onTap: count > 0 ? () => setState(() => _counts[m.id] = count - 1) : null,
                      child: Container(width: 36, height: 36, alignment: Alignment.center,
                          child: Icon(Icons.remove_circle_outline, size: 26,
                              color: count > 0 ? Theme.of(context).colorScheme.primary
                                  : Colors.grey.withOpacity(0.3))),
                    ),
                  ),
                  Container(
                    width: 36, height: 36, alignment: Alignment.center,
                    decoration: count > 0 ? BoxDecoration(
                        color: Theme.of(context).colorScheme.primaryContainer,
                        borderRadius: BorderRadius.circular(8)) : null,
                    child: Text('$count', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold,
                        color: count > 0 ? Theme.of(context).colorScheme.primary
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Total today', style: TextStyle(fontSize: 12, color: Colors.grey)),
                Text('$total meals', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              ]),
              FilledButton.icon(
              ...
              return Padding(
                ...
                child: Row(children: [
                  CircleAvatar(
                      radius: 14,
                      backgroundColor: Theme.of(context).colorScheme.primaryContainer,
                      child: Text(m.initials, style: TextStyle(fontSize: 10,
                          color: Theme.of(context).colorScheme.onPrimaryContainer))),
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        color: tot > 0 ? Theme.of(context).colorScheme.primaryContainer
                            : Colors.grey.withOpacity(0.1),
                        borderRadius: BorderRadius.circular(99)),
                    child: Text('$tot meals', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13,
                        color: tot > 0 ? Theme.of(context).colorScheme.primary : Colors.grey)),
                  ),
                ]),
              );
            }),
  ...
  Widget _buildCalendarView(ThemeData theme) {
    ...
    const headerBg = Color(0xFF1a1a2e);
    final altRowBg = isDark ? Colors.white.withOpacity(0.03) : Colors.black.withOpacity(0.02);
        ...
        child: Padding(
          ...
          child: Table(
            ...
            columnWidths: {
              ...
              ..._members.asMap().map((i, _) => MapEntry(i + 1, const FixedColumnWidth(60))),
            },
            children: [
              TableRow(
                ...
                children: [
                  ...
                  ..._members.map((m) => _cell(m.name, isHeader: true, small: true)),
                ],
              ),
                ...
                return TableRow(
                  ...
                  children: [
                    Padding(
                      ...
                      child: Text('$day $weekday', style: TextStyle(fontSize: 12,
                          color: isFriday ? Colors.orange.shade700 : theme.textTheme.bodySmall?.color)),
                    ),
                    ..._members.map((m) {
              ...
              TableRow(
                ...
                children: [
                  ...
                  ..._members.map((m) => _cell('${_totalForMember(m.id)}', isHeader: true)),
                ],
              ),
            ],
          ),
  ...
  Widget _cell(String text, {bool isHeader = false, bool small = false}) {
    return Padding(
  ...
  void _showDayEntryDialog(int day) {
    final year  = int.parse(widget.monthId.split('-')[0]);
    final month = int.parse(widget.monthId.split('-')[1]);
    setState(() { _date = DateTime(year, month, day); _showCalendar = false; });
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
...
class _BazarScreenState extends State<BazarScreen> {
  ...
  void _load() {
    ...
    setState(() { _entries = entries; _total = entries.fold(0.0, (s, e) => s + e.amount); });
  }
  ...
  void _save() {
    ...
    _db.saveBazar(BazarEntry(amount: amount, note: _noteCtrl.text.trim(),
        category: _category, date: _date, monthId: widget.monthId));
    _amountCtrl.clear(); _noteCtrl.clear(); _load();
  ...
  double _totalOnDay(int day) {
    final year  = int.parse(widget.monthId.split('-')[0]);
    final month = int.parse(widget.monthId.split('-')[1]);
    return _entries.where((e) => e.date.year == year && e.date.month == month && e.date.day == day)
  ...
  List<BazarEntry> _entriesOnDay(int day) {
    final year  = int.parse(widget.monthId.split('-')[0]);
    final month = int.parse(widget.monthId.split('-')[1]);
    return _entries.where((e) => e.date.year == year && e.date.month == month && e.date.day == day).toList();
  }
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      Card(
        child: Padding(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Total bazar this month', style: TextStyle(fontSize: 12, color: Colors.grey)),
              const SizedBox(height: 4),
              Text('৳${_total.toStringAsFixed(0)}',
                  style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
            ]),
            Text('${_entries.length} entries', style: const TextStyle(fontSize: 12, color: Colors.grey)),
          ]),
        ),
      ),
      const SizedBox(height: 10),
        ...
        child: Padding(
          ...
          child: Column(children: [
            ...
            SizedBox(width: double.infinity,
                child: FilledButton.icon(icon: const Icon(Icons.add_rounded, size: 18),
                    label: const Text('Add bazar entry'), onPressed: _save)),
          ]),
        ),
      ),
      const SizedBox(height: 16),
      if (_entries.isNotEmpty) ...[
        ...
        ..._entries.map((e) => Card(
          ...
          child: ListTile(
            leading: CircleAvatar(
              backgroundColor: Theme.of(context).colorScheme.primaryContainer, radius: 20,
              child: Text(e.category.substring(0, 1), style: TextStyle(fontSize: 13,
                  fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onPrimaryContainer)),
            ),
            title: Text(e.category, style: const TextStyle(fontWeight: FontWeight.w500)),
            ...
            trailing: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              IconButton(icon: const Icon(Icons.delete_outline, size: 18, color: Colors.red),
                  onPressed: () { _db.deleteBazar(e.id); _load(); }),
            ]),
          ),
        )),
      ] else
        Card(child: Padding(padding: const EdgeInsets.all(24),
          child: Center(child: Column(children: [
            Icon(Icons.shopping_basket_outlined, size: 40, color: Colors.grey.withOpacity(0.5)),
            const SizedBox(height: 8),
            const Text('No bazar entries yet', style: TextStyle(color: Colors.grey)),
  ...
  Widget _buildCalendarView() {
    ...
    return Column(children: [
      Container(
        width: double.infinity, color: headerBg,
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('Monthly bazar total', style: TextStyle(color: Colors.white70, fontSize: 13)),
          Text('৳${_total.toStringAsFixed(0)}',
      ...
      Expanded(child: SingleChildScrollView(
        ...
        child: Table(
          border: TableBorder.all(color: borderColor, width: 0.5),
          columnWidths: const {0: FixedColumnWidth(80), 1: FlexColumnWidth(1), 2: FixedColumnWidth(80)},
          children: [
            TableRow(decoration: const BoxDecoration(color: headerBg), children: [
            ...
            ...List.generate(days, (i) {
              ...
              if (!hasEntries) {
                final rowBg = isFriday ? Colors.orange.withOpacity(isDark ? 0.1 : 0.06)
                    : i.isOdd ? altRowBg : null;
                return TableRow(
                  decoration: rowBg != null ? BoxDecoration(color: rowBg) : null,
                  children: [
                    Padding(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 7),
                        child: Text('$day $weekday', style: TextStyle(fontSize: 12,
                            color: isFriday ? Colors.orange.shade700 : theme.textTheme.bodySmall?.color))),
                    Padding(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 7),
                        child: Text('—', style: TextStyle(fontSize: 12, color: Colors.grey.withOpacity(0.4)))),
                    Padding(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 7),
                        child: Text('—', style: TextStyle(fontSize: 12, color: Colors.grey.withOpacity(0.4)),
              ...
              return TableRow(
                decoration: BoxDecoration(color: Colors.green.withOpacity(isDark ? 0.1 : 0.06)),
                children: [
                  GestureDetector(
                    onTap: () => setState(() { _date = DateTime(year, month, day); _showCalendar = false; }),
                    child: Padding(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 7),
                        child: Text('$day $weekday', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600,
                            color: isFriday ? Colors.orange.shade700 : theme.textTheme.bodyMedium?.color))),
                  ),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 6),
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                        children: dayEntries.map((e) => Padding(padding: const EdgeInsets.only(bottom: 2),
                            child: Text('• ${e.category}${e.note.isNotEmpty ? ": ${e.note}" : ""}',
                                style: const TextStyle(fontSize: 12), overflow: TextOverflow.ellipsis))).toList()),
                  ),
                  Padding(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 7),
                      child: Text('৳${dayTotal.toStringAsFixed(0)}',
                          style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold,
                              color: Colors.green.shade700), textAlign: TextAlign.right)),
                ],
              );
            }),
            TableRow(decoration: const BoxDecoration(color: headerBg), children: [
              _calCell('Total', isHeader: true),
              _calCell('${_entries.length} entries', isHeader: true),
              _calCell('৳${_total.toStringAsFixed(0)}', isHeader: true),
            ]),
          ],
        ),
      )),
...
class _OtherCostScreenState extends State<OtherCostScreen> {
  ...
  void _save() {
    ...
    if (memberId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Please select a member first'), backgroundColor: Colors.red));
      return;
    }
    if (amount == null || amount <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Please enter a valid amount'), backgroundColor: Colors.red));
      return;
    }
    _db.saveCost(OtherCost(memberId: memberId, amount: amount, category: _category,
        note: _noteCtrl.text.trim(), monthId: widget.monthId));
    _amountCtrl.clear(); _noteCtrl.clear(); _load();
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cost added!')));
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          Padding(
            ...
            child: TextButton.icon(
              style: TextButton.styleFrom(foregroundColor: Colors.white),
              icon: Icon(_showSummary ? Icons.list_rounded : Icons.bar_chart_rounded, size: 18),
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      Card(
        ...
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('Total other costs', style: TextStyle(color: Colors.white70, fontSize: 13)),
      ...
      const SizedBox(height: 16),
      Text('BY MEMBER',
          style: theme.textTheme.labelSmall?.copyWith(color: Colors.grey, letterSpacing: 1)),
      const SizedBox(height: 8),
      Card(
        child: Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  ClipRRect(
                    ...
                    child: LinearProgressIndicator(value: pct, minHeight: 6,
                        backgroundColor: isDark ? Colors.white.withOpacity(0.08)
                  ...
                  Wrap(
                    ...
                    children: _categories
                        .where((cat) => _entries.any((e) => e.memberId == m.id && e.category == cat))
                        .map((cat) {
                      final amt = _entries.where((e) => e.memberId == m.id && e.category == cat)
                          .fold(0.0, (s, e) => s + e.amount);
                      return Container(
              ...
              Padding(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Wrap(spacing: 4, children: breakdown.entries.map((entry) {
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  String _dueLabel(int memberId) {
    ...
    final ms = s.members.firstWhere((m) => m.member.id == memberId,
        orElse: () => MemberSummary(member: Member(), totalMeals: 0, mealCost: 0,
  ...
  void _save() {
    ...
    if (memberId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Please select a member'), backgroundColor: Colors.red));
      return;
    }
    if (amount == null || amount <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
    ...
    _db.savePayment(Payment(memberId: memberId, amount: amount, method: _method,
        note: _noteCtrl.text.trim(), monthId: widget.monthId));
    _amountCtrl.clear(); _noteCtrl.clear(); _load();
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      if (_summary != null && _summary!.members.isNotEmpty) ...[
        ...
        ..._summary!.members.map((ms) {
          final color = ms.hasDue ? Colors.red : ms.isOverpaid ? Colors.blue : Colors.green;
          final label = ms.hasDue ? 'Owes ৳${ms.due.toStringAsFixed(0)}'
              : ms.isOverpaid ? 'Advance ৳${ms.due.abs().toStringAsFixed(0)}' : 'Settled';
          ...
          return Card(
            ...
            child: ListTile(
              ...
              leading: CircleAvatar(radius: 18, backgroundColor: color.withOpacity(0.15),
                  child: Text(ms.member.initials, style: TextStyle(
              ...
              trailing: Column(
                ...
                children: [
                  ...
                  if (ms.hasDue)
                    GestureDetector(
                      onTap: () { setState(() => _selectedMemberId = ms.member.id); _prefillDue(); },
                      child: Container(
      ...
      Card(
        child: Padding(
          ...
          child: Column(children: [
            if (_members.isNotEmpty)
              DropdownButtonFormField<int>(
                ...
                items: _members.map((m) => DropdownMenuItem(
                    value: m.id, child: Text('${m.name}${_dueLabel(m.id)}'))).toList(),
                onChanged: (v) { setState(() => _selectedMemberId = v); _prefillDue(); },
              ),
            const SizedBox(height: 10),
            TextField(controller: _amountCtrl, keyboardType: TextInputType.number,
                decoration: const InputDecoration(labelText: 'Amount (৳)')),
            ...
            const SizedBox(height: 12),
            SizedBox(width: double.infinity,
                child: FilledButton(onPressed: _save, child: const Text('Record payment'))),
          ]),
        ),
      ),
      const SizedBox(height: 16),
      Text('Payment log', style: theme.textTheme.titleSmall?.copyWith(color: Colors.grey)),
      const SizedBox(height: 8),
      if (_entries.isEmpty)
      ...
      ..._entries.map((e) {
        ...
        return Card(
          margin: const EdgeInsets.only(bottom: 6),
          child: ListTile(
            leading: CircleAvatar(radius: 18, backgroundColor: Colors.green.withOpacity(0.12),
                child: Text(member.initials, style: const TextStyle(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            if (s != null)
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                const Text('Still outstanding', style: TextStyle(color: Colors.white54, fontSize: 12)),
                Text('৳${s.totalDue.toStringAsFixed(0)}',
                    style: TextStyle(
                        color: s.totalDue > 0 ? Colors.red.shade300 : Colors.green.shade300,
          ...
          child: Column(
            children: _members.map((m) {
              ...
              final ms = s?.members.firstWhere((ms) => ms.member.id == m.id,
                  orElse: () => MemberSummary(member: m, totalMeals: 0, mealCost: 0,
                      otherCosts: 0, totalCost: 0, paid: 0, due: 0));
              final due = ms?.due ?? 0;
              final statusColor = (ms?.hasDue ?? false) ? Colors.red
                  : (ms?.isOverpaid ?? false) ? Colors.blue : Colors.green;
              ...
              return Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    CircleAvatar(radius: 16, backgroundColor: statusColor.withOpacity(0.15),
                        child: Text(m.initials, style: TextStyle(
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      if (ms != null)
                        Text('Cost ৳${ms.totalCost.toStringAsFixed(0)} · Paid ৳${paid.toStringAsFixed(0)}',
                            style: TextStyle(fontSize: 11, color: Colors.grey.shade600)),
                    ])),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                      decoration: BoxDecoration(
                          color: statusColor.withOpacity(0.1), borderRadius: BorderRadius.circular(99)),
                  ...
                  if (methodAmounts.isNotEmpty) ...[
                    ...
                    Wrap(spacing: 6, runSpacing: 4,
                        children: methodAmounts.entries.map((entry) => Container(
                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                          decoration: BoxDecoration(
                              color: Colors.green.withOpacity(isDark ? 0.15 : 0.10),
                              borderRadius: BorderRadius.circular(99)),
...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  void _load() { setState(() => _monthIds = _db.getAllMonthIds()); }
    ...
    return Scaffold(
      ...
      body: _monthIds.isEmpty
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Text('Add meals to start building history.', style: TextStyle(color: Colors.grey)),
          ])))
          : ListView.separated(
        padding: const EdgeInsets.all(14),
        ...
        itemBuilder: (_, i) {
          ...
          return Card(
            child: ListTile(
              ...
              trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                TextButton(onPressed: () => _exportSvc.shareJson(id), child: const Text('JSON')),
                TextButton(onPressed: () => _exportSvc.sharePdf(id), child: const Text('PDF')),
              ]),
            ),
          );
        },
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void _loadMembers() { setState(() => _members = _db.getActiveMembers()); }
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(14), children: [
        ...
        ..._members.map((m) => Card(
          margin: const EdgeInsets.only(bottom: 6),
          child: ListTile(
            leading: CircleAvatar(child: Text(m.initials, style: const TextStyle(fontSize: 12))),
            title: Text(m.name),
            ...
            trailing: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              IconButton(icon: const Icon(Icons.person_remove_outlined, size: 18, color: Colors.red),
        ...
        const SizedBox(height: 24),
        Center(child: Text('MessManager v1.0.0 · Local-first · ObjectBox',
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () {
              ...
              m.name = name; m.initials = init; m.isActive = true;
              m.joinedMonthId = _currentMonthId(); m.createdAt = DateTime.now();
              _db.saveMember(m);
              Navigator.pop(context); _loadMembers();
            },
  ...
  Future<void> _confirmSoftDelete(Member m) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('${m.name} will be hidden but all their data is preserved.'),
        actions: [
    ...
    if (ok == true) { _db.softDeleteMember(m.id); _loadMembers(); }
  }
  ...
  Future<void> _runImport() async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('This replaces all data for ${_formatMonthId(result.backup!.monthId)}.\n\nOther months are not affected.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
    ...
    if (ok == true && result.backup != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Import successful!'), backgroundColor: Colors.green));
        _loadMembers();
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Row(mainAxisSize: MainAxisSize.min, children: [
      IconButton(
        ...
        icon: const Icon(Icons.remove_circle_outline), iconSize: 22,
      ),
      SizedBox(width: 28, child: Text('$value', textAlign: TextAlign.center,
          style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold))),
      IconButton(
        onPressed: () => onChanged(value + 1),
        icon: const Icon(Icons.add_circle_outline), iconSize: 22,
      ),
    ]);
  }
}